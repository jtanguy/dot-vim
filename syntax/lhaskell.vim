...
if !exists("b:lhs_markup")
	...
	if lhs_markup =~ '\<\%(tex\|pandoc\|none\)\>'
	    let b:lhs_markup = matchstr(lhs_markup,'\<\%(tex\|pandoc\|none\)\>')
	else
    ...
    if b:lhs_markup !~ '\<\%(tex\|pandoc\|none\)\>'
	let b:lhs_markup = "unknown"
    endif
endif
...
elseif b:lhs_markup == "pandoc"
    if version < 600
	source <sfile>:p:h/pandoc.vim
	set isk+=_
    else
	runtime! syntax/pandoc.vim
	unlet b:current_syntax
    endif
    syntax cluster lhsTeXContainer contains=pandoc.*
else
